...
//XXX : Avoid coping code around. Maybe use defines.

void monoconv_bs1(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	lookups = 0;
	...
	assert_(i_mono_size[0] == 1);
	assert_(i_mono_size[1] == in_rows);
	assert_(i_mono_size[2] == in_cols);
	assert_(i_mono_size[3] == inner_depth);

	memset(out, 0, sizeof(float) * out_rows * out_cols * out_depth);
	// Color matrix multiplication.
	Eigen::Map<Eigen::MatrixXf>i_mat(i, in_rows * in_cols, in_depth);
	Eigen::Map<Eigen::MatrixXf>i_mono_mat(i_mono, in_rows * in_cols, inner_depth);
	...
	assert_(out_depth % inner_depth == 0);
	  ...
	  for (int n = 0; n < num_image_colors; ++n) { 
	    ...
	    for (int y = 0; y < out_cols; ++y) {
		  ...
		  Eigen::Map<Eigen::VectorXf>i_mono_vec(i_mono + x * stride + in_rows * (y * stride + py + d * in_cols), patch); 
		  Eigen::Map<Eigen::VectorXf>w_vec(w + patch * (py + in_d * patch), patch);
		  int out_offset = x + out_rows * (y + in_d * out_cols);
		  out[out_offset] += w_vec.transpose() * i_mono_vec; 
                }
	      }
	    }
	  }
	...
	Eigen::Map<Eigen::MatrixXf>out_mat(out, out_rows * out_cols, out_depth);
...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	int bs = i_size[0];
	if (bs == 1) { 
		monoconv_bs1(nlhs, plhs, nrhs, prhs);
	} else {
		monoconv(nlhs, plhs, nrhs, prhs);
	}
}
