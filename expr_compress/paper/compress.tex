...
Cost of multiplication $X$ with $\hat{W}$ is $O(nmt + nt^2 + ntk)$, which can be significantly smaller than $O(nmk)$ (for sufficiently small $t$).
It is worth to note, that often major cost is in memory management, and aforementioned complexities might be only relevant for large matrices.
