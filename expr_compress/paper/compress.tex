...
grows proportionally to the square size of local receptive field (in direct implementation). 
...
one has to perform Fourier transform, and inverse Fourier transform, which are
not without cost. They report speed up in forward pass up to $10x$ (depending on kernel size etc.).
Moreover, this method can be used jointly with most of techniques presented in this paper.



This paper get inspired by work of Denil et. al \cite{denil2013predicting} on redundancies in neural 
...
In this section, we give theoretical background on low rank approximations of
