...
/*
 * Block size #B_Yx#B_X. Each block applies #B_Y * #filtersPerThread filters to #B_X * #imgsPerThread images.
 ...
 void _filterActsMono(NVMatrix& images, NVMatrix& filters, NVMatrix& targets,
    ...
    if (numImgColors <= 4) {
       ...
       // printf("(G_X, G_Y) = (%d, %d) \n", DIVUP(numImages, #B_X * #imgsPerThread), (numModules * numFilters) / (#B_Y * #filtersPerThread));
        filterActsMonoEven_YxX_color <<<blocks, threads>>>(images.getDevData(), filters.getDevData(), targets.getDevData(), perm.getDevData(),
        ...
        filterActsMonoEvenManyCol_YxX_color <<<blocks, threads>>>(images.getDevData(), filters.getDevData(), targets.getDevData(), perm.getDevData(),
...
void monochromatic_input(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	filters->transpose();
	targets->transpose();
	_filterActsMono(*images, *filters, *targets,
                       imgSizeY, numModulesY, numModulesX, -paddingStart, moduleStride,
                       numImgColors, 0, 1, 1, *perm);
	images->transpose();
	filters->transpose();
	targets->transpose();
}
