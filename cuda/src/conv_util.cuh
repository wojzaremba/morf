...
void convContrastNormCrossMap(NVMatrix& images, NVMatrix& meanDiffs, NVMatrix& denoms, NVMatrix& target,
                             int numFilters, int sizeF, float k, float addScale, float powScale, bool blocked);
...
void convResponseNormCrossMap(NVMatrix& images, NVMatrix& denoms, NVMatrix& target, int numFilters, int sizeF, float k, float addScale,
...
void convLocalPool(NVMatrix& images, NVMatrix& target, int numFilters,
    ...
    assert_(images.getNumRows() == numFilters * imgPixels);
    int imgSize = int(sqrt(imgPixels));
    assert_(imgSize * imgSize == imgPixels);
    
    assert_(!images.isTrans());
    assert_(!target.isTrans());
    assert_(images.isContiguous());
//    assert_(numFilters % 4 == 0);
//    assert_(numImages % 128 == 0);
    ...
    if (strideX == 1 && subsX >= 6) {
        ...
        assert_((imgsPerThread * bx) % 32 == 0);
        assert_(numFilters % filtersPerThread == 0);
        dim3 threads(bx, 16);
...
__global__ void kNormalizeLCWeights(float* weights, const unsigned int numFilters, const int numModules, const unsigned int weightsPerFilter, const float norm) {
    const unsigned int moduleIdx = B_Y * blockIdx.y + threadIdx.y;
    const unsigned int filterIdx = B_X * blockIdx.x + threadIdx.x;
    
    float prod[filtersPerThread];
    #pragma unroll
    for (unsigned int i = 0; i < filtersPerThread; ++i) {
        prod[i] = 0;
    }
    if (moduleIdx < numModules) {
        weights += moduleIdx * weightsPerFilter * numFilters + filterIdx;
        for (unsigned int p = 0; p < weightsPerFilter; ++p) {
            #pragma unroll
            for (unsigned int i = 0; i < filtersPerThread; ++i) {
...
void normalizeLocalWeights(NVMatrix& weights, int numModules, float norm) {
    ...
    assert_(numModules * weightsPerFilter == weights.getNumRows());
    
    assert_(!weights.isTrans());
    assert_(weights.isContiguous());
...
__global__ void kCrop(float* imgs, float* target, const unsigned int numImages, const int imgStride,
                      const unsigned int imgSize, const unsigned int tgtSize, const unsigned int startY, const unsigned int startX) {
    const unsigned int imgPixels = imgSize * imgSize;
    const unsigned int tgtPixels = tgtSize * tgtSize;
    const unsigned int caseIdx = blockIdx.x * 32 * imgsPerThread + threadIdx.x;
    const unsigned int blockChanIdx = blockIdx.y / DIVUP(tgtPixels, 4);
    const unsigned int tgtPixelIdx = 4*(blockIdx.y % DIVUP(tgtPixels, 4)) + threadIdx.y;
    const unsigned int tgtPxY = tgtPixelIdx / tgtSize;
    const unsigned int tgtPxX = tgtPixelIdx % tgtSize;
    const unsigned int srcPixelIdx = (startY + tgtPxY) * imgSize + startX + tgtPxX;
    ...
    if (tgtPixelIdx < tgtPixels) {
        ...
        #pragma unroll
        for (unsigned int i = 0; i < imgsPerThread; ++i) {
...
void _convBedOfNails(NVMatrix& images, NVMatrix& target, int numChannels, int imgSize, int startX, int strideX,
    ...
    assert_(!target.isTrans());
    assert_(images.isContiguous());
    assert_(target.isContiguous());
    assert_(strideX > 1);
    
    int outputsX = DIVUP(imgSize, strideX);
    int outputs = outputsX * outputsX;
    if (reverse) {
        assert_(target.getNumRows() == numChannels * outputs);
    } else  {
        assert_(images.getNumRows() == numChannels * imgPixels);
    }
    ...
    if (scaleTargets == 0) {
        ...
        if (reverse) {
            assert_(images.getNumRows() == numChannels * outputs);
            assert_(images.getNumCols() == numImages);
        } else {
            assert_(target.getNumRows() == numChannels * outputs);
            assert_(target.getNumCols() == numImages);
        }
    }
...
void convGaussianBlur(NVMatrix& images, NVMatrix& filter, NVMatrix& target, bool horiz, int numChannels,
    ...
    assert_(imgPixels == imgSize * imgSize);
    assert_(radius >= 1 && radius <= 4);
    assert_(imgSize >= 2 * radius + 1);
    assert_(filter.getNumRows() == 1);
    assert_(images.getNumRows() == numChannels * imgPixels);
    assert_(!images.isTrans());
    assert_(!filter.isTrans());
    assert_(!target.isTrans());
    assert_(target.isContiguous());
    if (scaleTargets == 0) {
        target.resize(images);
    } else {
        assert_(target.isSameDims(images));
    }
...
__global__ void kFCNorm(float* imgs, float* meanDiffs, float* denoms, float* target, const int imgSize,
                                  const int numFilters, const int numImages, const int sizeF, const float k,
                                  const float addScale, const float powScale) {
    const int imgPixels = imgSize * imgSize;
    const int numImgBlocks = DIVUP(numImages, B_X*imgsPerThread);
...
void convLocalMaxUndo(NVMatrix& images, NVMatrix& maxGrads, NVMatrix& maxActs, NVMatrix& target,
    ...
    assert_(imgSize * imgSize == imgPixels);
    assert_(maxGrads.getNumRows() == numFilters * outputs);
    assert_(maxGrads.getNumCols() == numImages);
    assert_(!images.isTrans());
    assert_(!target.isTrans());
    assert_(!maxGrads.isTrans());
    assert_(!maxActs.isTrans());
    assert_(images.isContiguous());
    assert_(maxGrads.isContiguous());
    assert_(maxActs.isContiguous());
    assert_(maxGrads.isSameDims(maxActs));
    assert_(numFilters % 16 == 0);
//    assert_(numImages % 128 == 0);
    
    assert_(strideX <= subsX);
...
void convLocalAvgUndo(NVMatrix& avgGrads, NVMatrix& target,
    ...
    assert_(avgGrads.getNumRows() == numFilters * outputs);

    assert_(!target.isTrans());
    assert_(!avgGrads.isTrans());
    assert_(avgGrads.isContiguous());
    assert_(numFilters % 16 == 0);
//    assert_(numImages % 128 == 0);
...
void convContrastNorm(NVMatrix& images, NVMatrix& meanDiffs, NVMatrix& denoms, NVMatrix& target, int numFilters, int sizeX, float addScale, float powScale) {
    int numImages = images.getNumCols();
    int imgPixels = images.getNumRows() / numFilters;
    mexPrintf("getNumRows() = %d, getNumCols() = %d, numFilters = %d, imgPixels = %d\n", images.getNumRows(), images.getNumCols(), numFilters, imgPixels);
    assert_(images.getNumRows() == numFilters * imgPixels);
    int imgSize = int(sqrt(imgPixels));
    assert_(imgSize * imgSize == imgPixels);
    assert_(meanDiffs.isSameDims(images));
    
    assert_(!meanDiffs.isTrans());
    assert_(!images.isTrans());
    assert_(images.isContiguous());
    assert_(meanDiffs.isContiguous());
    assert_(numFilters % 16 == 0 || numFilters <= 8);
...
void convResponseNormUndo(NVMatrix& outGrads, NVMatrix& denoms, NVMatrix& inputs, NVMatrix& acts, NVMatrix& target, int numFilters,
    ...
    assert_(outGrads.getNumRows() == numFilters * imgPixels);
    
    assert_(denoms.isSameDims(outGrads));
    assert_(acts.isSameDims(denoms));
    assert_(!denoms.isTrans());
    assert_(!outGrads.isTrans());
    assert_(!acts.isTrans());
    assert_(!target.isTrans());
    assert_(outGrads.isContiguous());
...
void convRGBToYUV(NVMatrix& images, NVMatrix& target) {
    ...
    int numImages = images.getNumCols();
    assert_(images.getNumRows() == 3 * imgPixels);
...
void convCrop(NVMatrix& imgs, NVMatrix& target, int imgSize, int tgtSize, int startY, int startX) {
    ...
    assert_(imgs.getNumRows() == imgPixels * numChannels);
    assert_(imgPixels == imgSize * imgSize);
    assert_(imgSize - startY >= tgtSize);
    assert_(imgSize - startX >= tgtSize);
    assert_(startY >= 0);
    assert_(startX >= 0);
    target.resize(numChannels * tgtPixels, numImages);
    int imgsPerThread = numImages % 128 == 0 ? 4 : numImages % 64 == 0 ? 2 : 1;
...
void convTICAGrad(NVMatrix& images, NVMatrix& ticas, NVMatrix& target, int numFilters, int sizeX, float scaleTarget, float scaleOutput) {
    ...
    assert_(ticas.isSameDims(images));
    assert_(ticas.isContiguous());
...
void convContrastNormCrossMap(NVMatrix& images, NVMatrix& meanDiffs, NVMatrix& denoms, NVMatrix& target,
    ...
    assert_(imgSize * imgSize == imgPixels);
    assert_(meanDiffs.isSameDims(images));
    assert_(sizeF > 0 && sizeF <= numFilters);
    ...
    if (blocked) {
        if (checkCaseBounds) {
            ...
            kFCNorm<4, 32, 4, true, true><<<blocks, threads>>>(images.getDevData(), meanDiffs.getDevData(), denoms.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, sizeF, k, addScale, powScale);
        } else {
...
void convResponseNormCrossMap(NVMatrix& images, NVMatrix& denoms, NVMatrix& target, int numFilters, int sizeF, float k, float addScale, float powScale, bool blocked) {
    convContrastNormCrossMap(images, images, denoms, target, numFilters, sizeF, k, addScale, powScale, blocked);
}
